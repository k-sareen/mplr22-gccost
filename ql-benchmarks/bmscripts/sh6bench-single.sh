...
if [[ $# -ne 1 ]]; then
  echo -e "Usage: sh6bench-single.sh <num threads>"
  echo -e "    Wraps a single run of sh6bench with rss sampling\n"
  echo -e "    Example:"
  echo -e "        ./sh6bench-single.sh 1"
  exit 1
fi
...
BINARY=${SCRIPT_DIR}/../sh6bench

OUT_FILE=/tmp/sh6bench
TIMECMD_OUT=/tmp/sh6bench_run
